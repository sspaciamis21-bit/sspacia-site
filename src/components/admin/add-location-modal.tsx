...
export function AddLocationModal({ isOpen, onClose, onSuccess }: AddLocationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !slug || !cityId) {
      toast.error('Required node parameters missing.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(json.error ?? 'Node initialization failed');

      toast.success(`Node deployed: ${name}`);
      onSuccess();
      onClose();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'System initialization failed');
    } finally {
          ...
          >
            ...
            <div className="flex items-center justify-between px-10 py-8 border-b border-[var(--outline-variant)]/20 flex-shrink-0 animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-display font-black text-[#1B1C1C] tracking-tighter uppercase">Initialize Node</h2>
                  <p className="text-[10px] text-[#616161] font-bold uppercase tracking-[0.3em] mt-1 opacity-60">Registering new spatial coordinate</p>
                </div>
              </div>
              <button
                onClick={onClose}
              ...
              <form id="add-location-form" onSubmit={handleSubmit} className="space-y-12">
                ...
                <section className="space-y-8">
                  <div className="flex items-center gap-4 border-b border-[var(--outline-variant)]/20 pb-4">
                     <span className="text-[11px] font-black text-[#1B1C1C] uppercase tracking-[0.4em]">Spatial Registry</span>
                  </div>

                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                      <Label htmlFor="cityId" required>City Hub</Label>
                      <div className="relative">
                        <select
                          ...
                          required
                        >
                          <option value="">SELECT_HUB</option>
                          {cities.map((city) => (
                    ...
                    <div className="space-y-2">
                       <Label htmlFor="name" required>Node Designation</Label>
                       <input
                         id="name"
                         type="text"
                         value={name}
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                      <Label htmlFor="slug" required>Access Slug</Label>
                      <input
                        id="slug"
                        type="text"
                        value={slug}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="sortOrder">Priority Index</Label>
                      <input
                        id="sortOrder"
                        type="number"
                        value={sortOrder}
                ...
                <section className="space-y-8">
                   <div className="flex items-center gap-4 border-b border-[var(--outline-variant)]/20 pb-4">
                     <span className="text-[11px] font-black text-[#1B1C1C] uppercase tracking-[0.4em]">Signal Parameters</span>
                  </div>

                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                      <Label htmlFor="phone">Comms Frequency (Phone)</Label>
                      <input
                        id="phone"
                        type="text"
                        value={phone}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="address">Physical Coordinates (Address)</Label>
                    <textarea
                      id="address"
                      value={address}
                      onChange={(e) => setAddress(e.target.value)}
                      placeholder="Input absolute physical address..."
                      rows={3}
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                      <Label htmlFor="mapUrl">Nav Link (Google Maps)</Label>
                      <input
                        id="mapUrl"
                        type="url"
                        value={mapUrl}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="mapEmbed">Vector Map Embed</Label>
                      <input
                        id="mapEmbed"
                        type="text"
                        value={mapEmbed}
                    ...
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                         <p className="text-[11px] font-black text-[#1B1C1C] uppercase tracking-widest">
                           Node Operational Mode: {isActive ? 'BROADCASTING' : 'OFFLINE'}
                         </p>
                         <p className="text-[9px] text-[#616161] font-bold uppercase tracking-widest mt-1 opacity-60">
                           {isActive ? 'Publicly visible and indexable' : 'Hidden from registry'}
                         </p>
                       </div>
                    </div>
