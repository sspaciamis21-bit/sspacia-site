...
export function VisitorChatWidget() {
  ...
  useEffect(() => {
    ...
    const savedLastRead = localStorage.getItem("sspacia_visitor_last_read");
    if (savedLastRead) setLastReadAt(savedLastRead);
    if (savedToken) {
  ...
  // Poll messages: every 3s when open, every 6s in background when closed
  useEffect(() => {
    if (!sessionToken) return;

    fetchMessages();
    const intervalTime = isOpen ? 3000 : 6000;
    const interval = setInterval(fetchMessages, intervalTime);
    return () => clearInterval(interval);
  }, [sessionToken, isOpen, lastReadAt]);
  ...
  useEffect(() => {
    if (isOpen && sessionToken) {
      ...
      setLastReadAt(now);
      localStorage.setItem("sspacia_visitor_last_read", now);
    }
  }, [isOpen, sessionToken]);
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Calculate unread manager/admin messages if chat is currently closed
        if (!isOpen) {
          const currentLastRead = localStorage.getItem("sspacia_visitor_last_read");
          const unread = msgList.filter((m) => {
            if (m.senderType !== "ADMIN" && m.senderType !== "MANAGER") return false;
            if (!currentLastRead) return true;
            return new Date(m.createdAt) > new Date(currentLastRead);
          }).length;
  ...
  // Check if current user is staff (Community Manager, Super Admin, Admin) or in staff portal
  const userRole = (user?.role || "").toUpperCase();
  const isStaff = userRole === "ADMIN" || userRole === "SUPER_ADMIN" || userRole === "COMMUNITY_MANAGER" || userRole === "MANAGER";
  const isStaffPortal = pathname?.startsWith("/admin") || pathname?.startsWith("/manager");

  // NEVER show visitor chat or WhatsApp widget to staff (Community Manager / Super Admin) or in staff portals
  if (isStaff || isStaffPortal) {
    return null;
  }

  // For logged in regular users/members, show the floating WhatsApp widget
  if (isLoggedIn) {
    return (
