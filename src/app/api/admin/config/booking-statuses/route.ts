...
export const GET = withPermission('settings', 'read', async () => {
  try {
    const data = await prisma.bookingStatus.findMany({
      orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }],
    });
    return NextResponse.json({ data });
  } catch (error) {
    console.error('[BOOKING_STATUSES_READ]', error);
...
export const POST = withPermission('settings', 'update', async (req: NextRequest) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const created = await tx.bookingStatus.create({
        data: {
          name,
          displayName,
          color,
    ...
    console.error('[BOOKING_STATUSES_CREATE]', error);
...
export const PATCH = withPermission('settings', 'update', async (req: NextRequest) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const existing = await tx.bookingStatus.findUnique({ where: { id } });
      if (!existing) throw new Error('NOT_FOUND');

      const updated = await tx.bookingStatus.update({
        where: { id },
        data: {
          name: body.name,
    ...
    console.error('[BOOKING_STATUSES_UPDATE]', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
});
