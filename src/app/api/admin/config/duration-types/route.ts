...
export const GET = withPermission('settings', 'read', async () => {
  try {
    const data = await prisma.durationType.findMany({
      where: { isActive: true },
      orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }],
    });
    return NextResponse.json({ data });
  } catch (error) {
    console.error('[DURATION_TYPES_READ]', error);
...
export const POST = withPermission('settings', 'update', async (req: NextRequest) => {
  try {
    ...
    const { name, displayName, durationDays, durationHours, isActive, sortOrder } = body;
    ...
    const result = await prisma.$transaction(async (tx) => {
      const created = await tx.durationType.create({
        data: {
          name,
          displayName,
          slug,
    ...
    console.error('[DURATION_TYPES_CREATE]', error);
    if (error.code === 'P2002') return NextResponse.json({ error: 'Duration type already exists' }, { status: 400 });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
});
...
export const PATCH = withPermission('settings', 'update', async (req: NextRequest) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const existing = await tx.durationType.findUnique({ where: { id } });
      if (!existing) throw new Error('NOT_FOUND');

      const updated = await tx.durationType.update({
        where: { id },
        data: {
          name: body.name,
    ...
    console.error('[DURATION_TYPES_UPDATE]', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
});
