...
export const GET = withPermission('documents', 'view', async () => {
  try {
    ...
    console.error('[DOCUMENT_CATEGORIES_READ]', error);
...
export const POST = withPermission('documents', 'update', async (req: NextRequest) => {
  try {
    ...
    console.error('[DOCUMENT_CATEGORIES_CREATE]', error);
...
export const PATCH = withPermission('documents', 'update', async (req: NextRequest) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const existing = await tx.documentCategory.findUnique({ where: { id } });
      if (!existing) throw new Error('NOT_FOUND');

      const updated = await tx.documentCategory.update({
        where: { id },
        data: {
          name: body.name,
    ...
    console.error('[DOCUMENT_CATEGORIES_UPDATE]', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
});
