        ...
        <div className="p-6 flex items-center justify-between overflow-hidden">
          <AnimatePresence mode="wait">
            {isSidebarOpen ? (
              ...
              >
                SSPACIA <span className="text-[#004D40] capitalize">{user.role?.replace('_', ' ').toLowerCase() || 'Portal'}</span>
              </motion.div>
            ) : (
              <motion.div
                key="logo-mini"
          ...
          <div className="border-t border-[#CFD8DC]/30 p-4 shrink-0 flex flex-col gap-2">
            {isSidebarOpen ? (
              <div className="flex items-center gap-3 px-2 mb-2 w-full">
                <div className="h-10 w-10 shrink-0 rounded-lg bg-[#E0F7FA] text-[#006064] flex items-center justify-center border border-[#006064]/10 shadow-inner font-bold uppercase">
                  {user.name?.charAt(0)}
                </div>
